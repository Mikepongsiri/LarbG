...
function renderDateCarousel() {
    ...
    let today = new Date();
    // Default to the first date if nothing selected
    let firstDateStr = '';
    ...
    for (let i = 0; i < 14; i++) {
        ...
        let fullDateStr = `${y}-${m}-${day}`;
        
        if (i === 0) firstDateStr = fullDateStr;
        
        let isSelected = (selectedBookingDate === fullDateStr) ? 'card-active' : '';
        
        html += `
            <div class="date-card ${isSelected}" onclick="selectDate('${fullDateStr}')">
                <div>${daysArr[d.getDay()]}</div>
                <div>${d.getDate()}</div>
                <div>${monthsArr[d.getMonth()]}</div>
            </div>
        `;
    }
    ...
    if (!selectedBookingDate) {
        selectDate(firstDateStr);
    }
}

function renderTimeCarousel() {
    const container = document.getElementById('time-carousel-container');
    if (!container) return;
    ...
    let bookedTimes = workOrdersDB
        .filter(w => w.date === selectedBookingDate && w.status !== 'Cancelled')
        .map(w => w.time);
        
    let foundActive = false;
    
    timeSlots.forEach(t => {
        let isDisabled = bookedTimes.includes(t) ? 'card-disabled' : '';
        let isSelected = (!isDisabled && selectedBookingTime === t) ? 'card-active' : '';
        
        if (isSelected) foundActive = true;
        
        html += `
            <div class="time-card ${isDisabled} ${isSelected}" onclick="!this.classList.contains('card-disabled') && selectTime('${t}')">
                ${t}
            </div>
        `;
    });
    ...
    // If current selected time is now disabled or not selected, reset it
    if (!foundActive) {
...
function selectDate(dateStr) {
    ...
    renderDateCarousel(); // Re-render to update active class
    renderTimeCarousel(); // Re-render times because availability changed
    updateSelectionBanner();
}

function selectTime(timeStr) {
    selectedBookingTime = timeStr;
    renderTimeCarousel(); // Re-render to update active class
    updateSelectionBanner();
}

function updateSelectionBanner() {
    const banner = document.getElementById('selection-banner-container');
    if (!banner) return;
    
    if (selectedBookingDate && selectedBookingTime) {
        const parts = selectedBookingDate.split('-');
        const d = new Date(parseInt(parts[0]), parseInt(parts[1])-1, parseInt(parts[2]));
        const daysArrLong = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
        const monthsArrLong = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
        
        let displayStr = `${daysArrLong[d.getDay()]}, ${monthsArrLong[d.getMonth()]} ${d.getDate()} at ${selectedBookingTime} — click Confirm Booking to review.`;
        
        banner.innerHTML = `
            <div class="selection-banner" style="margin-top: 16px;">
                <div class="icon-check">✓</div>
                <div><strong>Selected:</strong> ${displayStr}</div>
            </div>
        `;
    } else {
        banner.innerHTML = `
            <div class="selection-banner" style="margin-top: 16px; opacity: 0.5; background: rgba(0,0,0,0.2); border-color: var(--border);">
                <div>Please pick a date and an available time slot above.</div>
            </div>
        `;
    }
}
        ...
        content = `
            <div class="glass-panel card" style="max-width: 600px; margin: 0 auto;">
                ...
                <form onsubmit="event.preventDefault(); showCustomerConfirmation();" id="booking-form">
                    ...
                    <div class="form-group">
                        <label style="margin-bottom: 8px;">Date</label>
                        <div id="date-carousel-container" class="slot-carousel">
                            <!-- Filled by JS -->
                        </div>
                    </div>
                    
                    <div class="form-group">
                        <label style="margin-bottom: 8px;">Time Slot</label>
                        <div id="time-carousel-container" class="slot-carousel">
                            <!-- Filled by JS -->
                        </div>
                    </div>
                    
                    <div id="selection-banner-container"></div>
                    ...
                    <div id="booking-error" style="display: none; background: rgba(239, 68, 68, 0.1); border-left: 3px solid var(--danger); padding: 16px; border-radius: 8px; margin-top: 16px;"></div>
...
function showCustomerConfirmation() {
    ...
    const date = selectedBookingDate || "TBD";
    const time = selectedBookingTime || "TBD";
    ...
    if (date === "TBD" || time === "TBD") {
        alert("Please explicitly select a valid date and available time slot before continuing.");
        return;
    }
    
    // Check available slots
    if (date !== "TBD" && time !== "TBD") {
        const MAX_SLOTS = 1; // Maximum capacity per slot
        const currentBookings = workOrdersDB.filter(w => w.date === date && w.time === time && w.status !== 'Cancelled');
        
        if (currentBookings.length >= MAX_SLOTS) {
            let errorContainer = document.getElementById('booking-error');
            if (errorContainer) {
                errorContainer.innerHTML = `
                    <h4 style="color: var(--danger); margin-top: 0; margin-bottom: 8px;">Slot Unavailable</h4>
                    <p style="font-size: 0.9rem; margin-bottom: 8px;">The selected time slot is fully booked. Please choose an available time slot from the carousel.</p>
                `;
                errorContainer.style.display = 'block';
            } else {
                alert("The selected time slot is fully booked.");
            }
            return; // Prevent booking confirmation
        }
    }
        ...
        <div class="glass-panel card" style="max-width: 600px; margin: 0 auto;">
            ...
            <form onsubmit="event.preventDefault(); saveNewPart();">
                <div class="form-group">
                    ...
                    <div id="code-error" style="display: none; color: var(--danger); font-size: 0.85rem; margin-top: 4px;"></div>
                </div>
                <div class="form-group">
...
function saveNewPart() {
    ...
    // E1 (Invalid Part Code) Pattern check: 1 Uppercase letter, hyphen, 3 digits
    const codeRegex = /^[A-Z]-\d{3}$/;
    if (!codeRegex.test(code)) {
        let errEl = document.getElementById('code-error');
        errEl.innerText = 'E1 (Invalid Part Code): Format must be 1 uppercase letter, a dash, and 3 digits (e.g. S-101).';
...
function generateQuotation(woId) {
    ...
    let partsCost = (wo.requiredParts || []).reduce((sum, p) => {
        let qty = typeof p === 'object' ? p.qty : 1;
        return sum + (qty * 75);
    }, 0); 
    let laborCost = (wo.estimatedLabor || 1.0) * 85; 
    
    let partsListHTML = (wo.requiredParts || []).map(p => {
        let code = typeof p === 'object' ? p.code : p;
        let qty = typeof p === 'object' ? p.qty : 1;
        return `<li>${qty}x ${code} - $${(qty * 75).toFixed(2)}</li>`;
    }).join('');
...
function startDiagnostic(jobId) {
    window.tempDiagFiles = []; // Reset temp media
    ...
    const content = `
        <div class="glass-panel card" style="max-width: 700px; margin: 0 auto;">
            ...
            <form onsubmit="event.preventDefault(); submitDiagnostic('${jobId}');">
                ...
                <div class="form-group">
                    <label>Media Evidence (Photos/Videos)</label>
                    <label id="diag-upload-zone" for="diag-media-upload" style="display: block; cursor: pointer; border: 2px dashed var(--border); border-radius: 8px; padding: 16px; text-align: center; min-height: 100px; background: rgba(0,0,0,0.2); transition: all 0.3s ease;">
                        <input type="file" id="diag-media-upload" accept="image/*,video/*" multiple style="display:none;" onchange="handleDiagFileUpload(event)">
                        <div id="diag-upload-icon" style="font-size: 2rem; color: rgba(255,255,255,0.5); margin-bottom: 8px;">📸</div>
                        <div id="diag-upload-text" style="color: var(--text-muted);">
                            <strong>Select photos or videos</strong><br>
                            <span style="font-size:0.8rem">Visual proof of identified issues</span>
                        </div>
                    </label>
                    <div id="diag-media-list" style="margin-top: 12px; display: flex; flex-direction: column; gap: 8px;"></div>
                </div>
                
                <div class="form-group">
                    <label>Required Parts</label>
                    <div id="parts-list-container" style="margin-bottom: 12px;">
                        <!-- Rows injected here -->
                    </div>
                    <button type="button" class="btn secondary" style="align-self: flex-start;" onclick="addDiagnosticPartRow()">+ Add Part</button>
                </div>
    ...
    `;
    document.getElementById('main-content').innerHTML = content;
    addDiagnosticPartRow(); // Add first empty row by default
}

function addDiagnosticPartRow() {
    const list = document.getElementById('parts-list-container');
    if (!list) return;
    const rowId = 'part-row-' + Math.floor(Math.random()*10000);
    const rowHtml = `
        <div id="${rowId}" style="display: flex; gap: 12px; margin-bottom: 12px; align-items: center;">
            <div style="flex: 2;">
                <input type="text" class="form-control part-code-input" placeholder="Part Code (e.g. B-001)" style="width: 100%;">
            </div>
            <div style="flex: 1;">
                <input type="number" class="form-control part-qty-input" placeholder="Qty" value="1" min="1" style="width: 100%;">
            </div>
            <button type="button" class="btn danger" style="padding: 10px 14px; background: rgba(239, 68, 68, 0.2); color: #ef4444;" onclick="document.getElementById('${rowId}').remove()">✕</button>
        </div>
    `;
...
function submitDiagnostic(woId) {
    ...
    if(wo) {
        ...
        wo.diagnosticMedia = window.tempDiagFiles || [];
        
        let requiredParts = [];
        const container = document.getElementById('parts-list-container');
        if (container) {
            const codeInputs = container.querySelectorAll('.part-code-input');
            const qtyInputs = container.querySelectorAll('.part-qty-input');
            
            for (let i = 0; i < codeInputs.length; i++) {
                let code = codeInputs[i].value.trim();
                let qty = parseInt(qtyInputs[i].value, 10);
                if (isNaN(qty) || qty < 1) qty = 1;
                
                if (code) {
                    requiredParts.push({ code, qty });
                }
            }
        }
...
function handleDiagFileUpload(event) {
    const files = event.target.files;
    const list = document.getElementById('diag-media-list');
    
    if(!window.tempDiagFiles) window.tempDiagFiles = [];
    
    for(let i=0; i<files.length; i++) {
        let file = files[i];
        window.tempDiagFiles.push(file.name);
        
        let typeIcon = file.type.includes('video') ? '🎥' : '📷';
        list.insertAdjacentHTML('beforeend', `
            <div style="display: flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.05); padding: 8px 12px; border-radius: 6px; font-size: 0.85rem;">
                <div><span style="margin-right: 8px;">${typeIcon}</span>${file.name}</div>
                <span style="color: var(--success); font-size: 0.8rem;">Ready for upload</span>
            </div>
        `);
    }
    
    if (window.tempDiagFiles.length > 0) {
        document.getElementById('diag-upload-icon').innerText = '✅';
        document.getElementById('diag-upload-icon').style.color = 'var(--accent)';
        document.getElementById('diag-upload-text').innerHTML = `<strong>${window.tempDiagFiles.length} files selected</strong><br><span style="font-size:0.8rem">Click to add more</span>`;
        document.getElementById('diag-upload-zone').style.borderColor = 'var(--accent)';
        document.getElementById('diag-upload-zone').style.background = 'rgba(16, 185, 129, 0.1)';
    }
}
